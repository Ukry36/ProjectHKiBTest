...
public class ResistanceBuffType : StatBuffTypeSO
{
    public override void AddBuff(IInterfaceRegistable registable, StatBuffSO buff, int effectIndex, Gear sourceGear, int multiplyer, bool stack)
    {
        ...
        {
            damagable.ResistanceBuffer.AddBuff(buff, effectIndex, sourceGear, multiplyer, stack);
        }
    }
    ...
    public override void RemoveBuff(IInterfaceRegistable registable, StatBuffSO buff, int effectIndex, Gear sourceGear, int multiplyer, bool remove)
    {
        if (registable is InterfaceRegister interfaceRegister &&
            interfaceRegister.TryGetInterface(out IDamagable damagable))
        {
            damagable.ResistanceBuffer.RemoveBuff(buff, effectIndex, sourceGear, multiplyer, remove);
        }
    }
}