...
public class ParticleManager : PoolManager<ParticlePlayer>
{
    ...
    {
        var clone = ReuseObject(ID, transform, quaternion.identity, attatchToTransform, false);
        if (clone.TryGetComponent(out ParticlePlayer ParticlePlayer))
        {
    ...
    {
        ParticlePlayer clone = ReuseObject(ID, transform, quaternion.identity, attatchToTransform, false);
        if (!clone)
        {
    ...
    {
        ParticlePlayer clone = ReuseObjectOneShot(ID, pos, quaternion.identity, false);
        if (!clone)
        {
            Debug.LogError("ERROR: Failed to play Particle(failed to reuse object)!!! ID: " + ID);
            return null;
