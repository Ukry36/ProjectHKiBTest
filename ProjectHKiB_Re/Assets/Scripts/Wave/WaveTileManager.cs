...
public class TilePosInfo
{
    ...
    [field: SerializeField] public Vector3Int QPos { get; private set; }
    ...
    public TilePosInfo(Vector2 pos, Vector3Int qPos, Vector3Int frontWallPos, Vector3Int rearWallPos, bool isWall, float placeOrder, TileBase quantumTile, TileBase outlineTile, TileBase frontVoidTile, TileBase rearVoidTile, Matrix4x4 flipInfo, Transform quantumDecoration, Transform frontDecoration, Transform rearDecoration)
    {
        Pos = pos;
        QPos = qPos;
    ...
    {
        quantumTilemap.SetTile(QPos, frontOrRear && QuantumTile ? QuantumTile : null);
        quantumTilemap.SetTransformMatrix(QPos, FlipInfo);
        outlineTilemap.SetTile(QPos, !(frontOrRear && QuantumTile) ? OutlineTile : null);
    ...
    public void InitTile(Tilemap quantumTilemap, Tilemap outlineTilemap, EntityWallGrid frontWall, EntityWallGrid rearWall, bool isDirectionForward)
    {
        quantumTilemap.SetTile(QPos, null);
        quantumTilemap.SetTransformMatrix(QPos, FlipInfo);
        outlineTilemap.SetTile(QPos, OutlineTile);
        if (QuantumDecoration != null)
...
public class WaveTileManager : MonoBehaviour
{
    ...
    {
        TileInfoList = new();
        Vector2 max = _wallColider.bounds.max - Vector3.one * 0.5f;
        Vector2 min = _wallColider.bounds.min + Vector3.one * 0.5f;
            ...
            for (float y = min.y; y < max.y; y++)
            {
                ...
                TileInfoList.Add(new TilePosInfo
                    (
                        ...
                        frontWallPos,
                        rearWallPos,
                        Physics2D.OverlapPoint(_quantumTilemap.CellToWorld(qPos), layerMask),
    ...
    public void InitTileMap(List<TilePosInfo> tilePosInfoList, bool isDirectionForward)
    {
        ...
        {
            tilePosInfoList[i].InitTile(_quantumTilemap, _outlineTilemap, _frontEntityWallGrid, _rearEntityWallGrid, isDirectionForward);
        }
    }
    ...
    private IEnumerator PlaceOneWaveTiles(int currentWaveIndex, int waveCount, bool frontOrRear, bool isDirectionForward)
    {
        ...
        for (int i = 0; i < _stepPerWave; i++)
        {
            ...
            for (int j = 0; j < tilePerStep; j++)
            {
                ...
                OnPlaceTile(_quantumTilemap.CellToWorld(tile.QPos), frontOrRear);
            ...
            if (i == _stepPerWave - 1)
            {
                for (int k = 0; k < WaveSeparatedTileInfo[currentWaveIndex].Count; k++)
                {
                    ...
                    OnPlaceTile(_quantumTilemap.CellToWorld(WaveSeparatedTileInfo[currentWaveIndex][k].QPos), frontOrRear);
                    if (k % div == 0)
                    {
                        OnPlaceTileReduced(frontOrRear);
                        yield return null;
